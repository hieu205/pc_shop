...
export const useProducts = (
  ...
  return useQuery({
    ...
    queryFn: async () => {
      ...
      if (catIds.length === 1) {
        // If there's a search term, prefer generic endpoint so backend can filter by name within the category
        if (filters.search) {
        ...
        // No search: use dedicated category endpoint (usually faster/simpler)
