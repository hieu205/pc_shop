...
export const exportQuotationExcel = (selected: SelectedParts, totals: PriceTotals, meta: Record<string, string> = {}) => {
    const items: Product[] = Object.values(selected).filter(Boolean) as Product[];
    const rows = [];
  ...
  `;
    ...
    const itemsHtmlRows = items.map((it, i) => {
        return `<tr>
      <td style="border:1px solid #000;padding:6px;text-align:center">${i + 1}</td>
      <td style="border:1px solid #000;padding:6px">${escapeHtml('P-' + it.id)}</td>
      <td style="border:1px solid #000;padding:6px">${escapeHtml(it.name)}</td>
      <td style="border:1px solid #000;padding:6px;text-align:center">Theo NSX</td>
      <td style="border:1px solid #000;padding:6px;text-align:center">1</td>
      <td style="border:1px solid #000;padding:6px;text-align:right">${escapeHtml(fmtMoney(it.price))}</td>
      <td style="border:1px solid #000;padding:6px;text-align:right">${escapeHtml(fmtMoney(it.price))}</td>
    </tr>`;
    }).slice(0, 10);
