...
export const initializeCart = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue('Failed to initialize cart state');
    }
  }
);
        ...
        if (action.payload && (action.payload as any).cart_items) {
          ...
          try {
            ...
            state.summary = {
              ...
              total_amount: (cartData.total_amount || 0) - (cartData.discount_amount || 0),
              final_amount: cartData.final_amount || ((cartData.total_amount || 0) - (cartData.discount_amount || 0)),
            };
          } catch (e) {
