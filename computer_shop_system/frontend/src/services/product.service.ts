...
// Detect and remember local backend "List parameter name" binding bug for /products endpoint
let HAS_LIST_PARAM_BINDING_BUG = false;
try {
  const persisted = sessionStorage.getItem('has_list_param_binding_bug');
  HAS_LIST_PARAM_BINDING_BUG = persisted === '1';
} catch (_) { /* ignore */ }

const markListParamBindingBug = () => {
  HAS_LIST_PARAM_BINDING_BUG = true;
...
export const productService = {
  ...
  getAllProducts: async (
    ...
    try {
      // Normalize sort fields from camelCase -> snake_case when necessary
      // Backend expects fields like 'updated_at', 'created_at', 'low_stock_threshold', etc.
      const paramsCopy = { ...(params || {}) } as any;
      if (paramsCopy.sort && typeof paramsCopy.sort === 'string') {
        // support sort like 'updatedAt,desc' or 'name,asc' or 'updated_at,desc'
        const parts = paramsCopy.sort.split(',');
        const field = parts[0];
        const dir = parts[1] || '';

        // Map common fields to backend-supported fields
        const fieldMap: { [key: string]: string } = {
          'updated_at': 'name', // Fallback to name if updated_at not supported
          'created_at': 'name', // Fallback to name if created_at not supported
          'name': 'name',
          'price': 'price',
          'quantity': 'quantity'
        };

        // If field already contains an underscore, assume it's snake_case and keep it.
        let snake = field;
        if (!field.includes('_')) {
          // convert camelCase -> snake_case
          snake = field.replace(/([A-Z])/g, (m: string) => `_${m.toLowerCase()}`);
        }

        // Use mapped field if available, otherwise use the converted field
        const mappedField = fieldMap[field] || snake;
        paramsCopy.sort = [mappedField, dir].filter(Boolean).join(',');
      } else {
        // Default sort if none provided
        paramsCopy.sort = 'name,asc';
      }

      // Convert any array parameters to comma-separated strings to avoid sending repeated
      // query keys (categoryIds=1&categoryIds=2) which some local backend builds may not bind.
      ...
      // If we've detected local backend binding bug, skip calling /products and go straight to fallbacks
      if (!HAS_LIST_PARAM_BINDING_BUG) {
        const response = await api.get<{ content: Product[]; totalElements: number; totalPages: number; }>('/products', { params: safeParams });
        return response.data;
      }
      // force jump to catch for fallback flow
      throw { status_code: 400, message: 'Skip /products due to local List binding bug' };
    } catch (error: any) {
          ...
          if (isBindingProblem && arrayKey && arrayKey.toLowerCase().includes('category')) {
            ...
            if (ids.length > 0) {
              ...
              const fetches = ids.map(id =>
                api
                  .get<{ content: Product[]; totalElements: number; totalPages: number; }>(`/products/category/${id}`, { params: { page, size } })
                  .then(r => r.data)
                  .catch((e) => {
              ...
              if (total > 0) {
                return { content: unique, totalElements: total, totalPages };
              }
            }
          }
        }
      ...
      if (!STRICT_BACKEND) try {
        if (isBindingProblem && !HAS_PATHVAR_BINDING_BUG) {
          ...
          const fetches = topCategories.map((c) =>
            api
              .get<{ content: Product[]; totalElements: number; totalPages: number }>(`/products/category/${(c as any).id}`, {
          ...
          if (total > 0) {
            return { content: paged, totalElements: total, totalPages };
          }
        }
      } catch (aggregateErr) {
      ...
      if (!STRICT_BACKEND) try {
        ...
        const searchResp = await api.get<{ content: Product[]; totalElements: number; totalPages: number; }>(
          '/products/search',
          { params: { keyword, page, size } }
        ).catch(() => null);
        if (searchResp && searchResp.data) return searchResp.data;
      } catch (searchErr) {
      ...
      try {
        ...
        const retryResp = await api.get<{ content: Product[]; totalElements: number; totalPages: number; }>('/products', { params: paramsCopy }).catch(() => null);
        if (retryResp && retryResp.data) return retryResp.data;
      } catch (retryErr) {
  ...
  const response = await api.get<{ content: Product[]; totalElements: number; totalPages: number; }>(`/products/category/${categoryId}`, { params: safeParams });
  ...
  const response = await api.get<{ content: Product[]; totalElements: number; totalPages: number; }>('/products/search', {
        params: safeParams
      });
   ...
   */
  createProduct: async (productData: CreateProductRequest, options?: { images?: File[]; primaryImageIndex?: number }): Promise<Product> => {
    try {
  // If images provided, perform multipart upload flow
  if (options?.images && options.images.length > 0) {
    const form = new FormData();
    form.append('metadata', new Blob([JSON.stringify(productData)], { type: 'application/json' }));
