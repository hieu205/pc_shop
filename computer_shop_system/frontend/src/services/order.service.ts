...
export const orderService = {
   ...
   */
  getUserOrders: async (userId: number, opts?: {
    page?: number;
    size?: number;
    status?: string
  }): Promise<Page<Order>> => {
    try {
      console.log(`📦 Order Service: Đang lấy đơn hàng của người dùng ${userId}...`);
      const { page = 0, size = 10, status } = opts || {};
      const params: any = { page, size };
      if (status) params.status = status;

      const resp: any = await api.get(`/orders/user/${userId}`, { params });
      const raw = resp && (resp as any).data ? (resp as any).data : resp;
      if (Array.isArray(raw)) {
        return { content: raw as Order[], totalElements: raw.length, totalPages: 1 } as Page<Order>;
      }
      ...
      const pageObj = raw?.content ? raw : raw?.data ? raw.data : raw;
      const content = (pageObj?.content ?? pageObj?.items ?? pageObj) as Order[];
      const totalElements = pageObj?.totalElements ?? pageObj?.total_elements ?? pageObj?.total ?? (Array.isArray(content) ? content.length : 0);
      ...
      return {
        content: normalizedContent as Order[],
        totalElements: Number(totalElements ?? 0),
        totalPages: Number(totalPages ?? 0),
      } as Page<Order>;
    } catch (error: any) {
  ...
  /**
   * Create order from cart - POST /api/v1/orders/user/{userId}/from-cart
   */
  createOrderFromCart: async (userId: number, orderData: CreateOrderRequest): Promise<Order> => {
    try {
      console.log(`📦 Order Service: Đang tạo đơn hàng từ giỏ hàng cho người dùng ${userId}...`);
      const resp: any = await api.post(`/orders/user/${userId}/from-cart`, orderData);
      const payload = resp && (resp as any).data ? (resp as any).data : resp;
